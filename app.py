...
from core.sync_engine import bootstrap_from_cloud_once, load_sync_meta
...
# V8.2: show cloud-persistence danger immediately, not hidden in Obsidian module.
try:
    _cfg = load_github_config(st.secrets)
    _meta = load_sync_meta()
    if not _cfg.is_ready:
        st.error("⚠️ Cloud persistence is OFF: GITHUB_TOKEN / GITHUB_REPO are missing. Changes can disappear after Streamlit reboot.")
    elif _meta.get("last_bootstrap_error"):
        st.warning(f"⚠️ Cloud bootstrap warning: {_meta.get('last_bootstrap_error')}")
except Exception:
    pass
