...
def save_json(path, data):
    ...
    # V8.1: cloud-first auto-push. This is intentionally best-effort:
    # local use still works without secrets, and failures are recorded in sync_meta.json.
    try:
        path_obj = Path(path)
        if path_obj.name in {"tasks.json", "journals.json"}:
            from core.sync_engine import auto_push_after_local_save
            auto_push_after_local_save(trigger=path_obj.name)
    except Exception:
        pass
...
def _ensure_task_defaults(task):
    ...
    if not task.get("obsidian_file"):
        task["obsidian_file"] = make_obsidian_tracker_path(task)
        changed = True
...
def make_obsidian_tracker_path(task):
    title = task.get("title", "Untitled")
    task_id = str(task.get("id") or "no-id")
    short_id = task_id.split("-")[0][:8] or task_id[:8] or "no-id"
    return f"Trackers/{slugify(title)}--{short_id}.md"
