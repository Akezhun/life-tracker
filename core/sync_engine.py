...
from core.markdown_export import build_vault_files
...
def build_all_vault_files(journals: List[dict] | None = None, tasks: List[dict] | None = None) -> Dict[str, str]:
    ...
    files.update(_data_files(tasks, journals))
    return files
...
def bootstrap_from_cloud_once():
    """On Streamlit Cloud, hydrate local JSON from GitHub once per session."""
    if st is None:
        return
    try:
    ...
    try:
        ...
        if journals_text:
            journals = json.loads(journals_text)
